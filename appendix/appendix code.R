...
t4 <- "https://github.com/thomasjwood/false_alarm/raw/master/appendix/t3.RDS" %>% 
  url %>% 
  gzcon %>% 
  readRDS %>% 
  tbl_df %>% 
...
# p59, p60, p61, p62, p63
#  These tables are replicated with the data and syntax here
# https://dataverse.harvard.edu/dataset.xhtml?persistentId=doi:10.7910/DVN/JHOK6X
# 

# p65
"C:/Dropbox/false alarm/replication data and code/appendix/t5.5.rds" %>% 
  readRDS %>% 
  lm %>% 
  stargazer(
    type = "text",
    initial.zero = F, 
    covariate.labels = str_c(
      1:3 %>% 
        rep(times = 2),
      " ",
      c("Correction",
...
model.1 <- lm(generalapproval ~ correction1 + correction2 + correction3 + misinfo1 + misinfo2 + misinfo3, data=notestdata2_0)

stargazer(model.1, title="Approval Results")
...
str_c(
  ...
  map(
    ~lm(
      .,
      data = "C:/Dropbox/false alarm/replication data and code/appendix/t6.rds" %>% 
        readRDS
    )
  ) %>% 
  stargazer(
...
"C:/Dropbox/false alarm/replication data and code/appendix/t5.rds" %>% 
  readRDS  %>%
  group_by(
    item
  ) %>% 
