...
t1 <- "C:/Dropbox/false alarm/replication data and code/chapter 1/USMISC2015-MIPD_ind.rds" %>%
  readRDS %>% 
  select(
    studyid, id,
    fw_end,
...
t3 <-  "C:/Dropbox/false alarm/replication data and code/chapter 1/study 1 modelling table.RDS" %>% 
  readRDS
...
t5 <- "C:/Dropbox/false alarm/replication data and code/chapter 1/study 2 modelling table.RDS" %>% 
  readRDS
...
t6$speaker <- case_when(
  t6$item %>%
    ...
    str_sub(2, -2) %>% 
    is_in(
      c("Obama",  "Gutiérrez",  "Longoria", "Lee") 
    ) ~ "Liberal Speaker",
  t6$item %>%
    str_extract("\\(\\w{3,10}\\)") %>%
    str_sub(2, -2) %>% 
  ...
  geom_text(
    ...
    data = t6_l_2 %>% 
      filter(
        ideol == "Liberals" &
          item2 %>% 
          str_detect("Obama restricts drones' use")
      ) %>% 
      select(item2, ideol) %>% 
      mutate(
...
# t5 <- "C:/Dropbox/EthanandTom/corrections/data/6 study correction modelling table.csv" %>% 
#   read_csv  %>% 
#   filter(
#     study == 3
#   )
# t5 %>% 
#   saveRDS(
#     "C:/Dropbox/false alarm/replication data and code/chapter 1/study 3 modelling table.RDS"
#   )

t7 <- "C:/Dropbox/false alarm/replication data and code/chapter 1/study 3 modelling table.RDS" %>% 
  readRDS
...
t9 <- "C:/Dropbox/false alarm/replication data and code/chapter 1/study 4 modelling table.RDS" %>% 
  readRDS
...
t15 <- "C:/Dropbox/false alarm/replication data and code/chapter 1/study 5 modelling table.RDS" %>% 
  readRDS
...
t18 <- "C:/Dropbox/false alarm/replication data and code/chapter 1/study 6 modelling table.RDS" %>% 
  readRDS
...
t20 <- "C:/Dropbox/false alarm/replication data and code/chapter 1/study 7 modelling table.RDS" %>% 
  readRDS

t20$it_join <- t20$item %>% 
...
ggplot() +
  ...
  geom_text(
    aes(mu2, item, label = lab, color = source),
    data = lt3,
    family = "Roboto",
    size = 2
  ) +
...
t22 <- "C:/Dropbox/false alarm/replication data and code/chapter 1/study 8 modelling table.RDS" %>% 
  readRDS
...
t22 %<>% 
  {
    ...
    j <- "C:/Dropbox/false alarm/replication data and code/chapter 1/study 9 modelling table.RDS" %>% 
      readRDS
...
t24 %>% 
  ...
  labs(
    ...
    y = ""
    # title = "Correction effects by political period",
    # subtitle = "Hollow points indicate insignficant corrections (p >= .05)"
  ) +
  theme_minimal(base_family = "Roboto") +
  theme(
    axis.text.y = element_text(
